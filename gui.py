...
class App:
    ...
    def setup_mercado(self):
        ...
        # Busca sequencial e binária no mercado (exemplo de uso)
        busca_frame_seq = ttk.Frame(self.tab_mercado)
        busca_frame_seq.pack(pady=5)
        ttk.Label(busca_frame_seq, text="Buscar ação (Sequencial):").pack(side=tk.LEFT)
        self.busca_seq_var = tk.StringVar()
        ttk.Entry(busca_frame_seq, textvariable=self.busca_seq_var, width=15).pack(side=tk.LEFT, padx=2)
        ttk.Button(busca_frame_seq, text="Buscar", command=self.buscar_acao_sequencial_popup).pack(side=tk.LEFT, padx=2)

        busca_frame_bin = ttk.Frame(self.tab_mercado)
        busca_frame_bin.pack(pady=5)
        ttk.Label(busca_frame_bin, text="Buscar ação (Binária):").pack(side=tk.LEFT)
        self.busca_bin_var = tk.StringVar()
        ttk.Entry(busca_frame_bin, textvariable=self.busca_bin_var, width=15).pack(side=tk.LEFT, padx=2)
        ttk.Button(busca_frame_bin, text="Buscar", command=self.buscar_acao_binaria_popup).pack(side=tk.LEFT, padx=2)
    ...
    def comprar_acao(self, nome):
        # Usa busca sequencial para encontrar a ação pelo nome
        acao = self.buscar_acao_sequencial(nome)
        if not acao:
            messagebox.showerror("Erro", "Ação não encontrada (busca sequencial)!")
            return
        acao_obj = self.mercado.get_acao_por_nome(acao[0])
    ...
    def vender_acao(self, nome):
        # Usa busca binária para encontrar a ação pelo nome
        acao = self.buscar_acao_binaria(nome)
        if not acao:
            messagebox.showerror("Erro", "Ação não encontrada (busca binária)!")
            return
        acao_obj = self.mercado.get_acao_por_nome(acao[0])
    ...
    def buscar_acao_sequencial(self, nome):
        """Busca uma ação pelo nome usando busca sequencial."""
        acoes = self.mercado.listar_acoes()  # [(nome, preco), ...]
        nomes = [n for n, _ in acoes]
        idx = buscar('sequencial', nomes, nome)
        return acoes[idx] if idx != -1 else None

    def buscar_acao_binaria(self, nome):
        """Busca uma ação pelo nome usando busca binária (lista ordenada)."""
        acoes = sorted(self.mercado.listar_acoes(), key=lambda x: x[0])  # Ordena por nome
        nomes = [n for n, _ in acoes]
        idx = buscar('binaria', nomes, nome)
        return acoes[idx] if idx != -1 else None

    def buscar_acao_sequencial_popup(self):
        nome = self.busca_seq_var.get()
        resultado = self.buscar_acao_sequencial(nome)
        if resultado:
            messagebox.showinfo("Busca Sequencial", f"Ação encontrada: {resultado[0]} - ${resultado[1]:.2f}")
        else:
            messagebox.showwarning("Busca Sequencial", "Ação não encontrada.")

    def buscar_acao_binaria_popup(self):
        nome = self.busca_bin_var.get()
        resultado = self.buscar_acao_binaria(nome)
        if resultado:
            messagebox.showinfo("Busca Binária", f"Ação encontrada: {resultado[0]} - ${resultado[1]:.2f}")
        else:
